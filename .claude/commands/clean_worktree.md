...
2. **Show any uncommitted changes** (safety check):
   ```bash
...
3. **Show any unpushed commits** (safety check):
   ```bash
   if [ -d "trees/<worktree_name>" ]; then
     cd trees/<worktree_name>
...
4. **Remove the worktree** (if exists):
   ```bash
...
8. **Verify cleanup**:
   ...
   git worktree list | grep <worktree_name> || echo "Worktree removed"

   # Verify branch is gone
   git branch --list <worktree_name> || echo "Branch removed"
   ```
...
- The `--force` flag ensures cleanup works even with uncommitted changes
