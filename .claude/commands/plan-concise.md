...
- For event-driven features: explicitly trace cancellation paths to determine where timers/state should live (frontend vs backend)
...
<if feature involves events, timers, or async operations, include this section:>
## Event Flow & State Ownership
For each event/signal in the system, document:

| Event | Emitter | Receiver | State Changes |
|-------|---------|----------|---------------|
| <event name> | <who emits> | <who handles> | <what state changes> |

### Cancellation Paths
For any delayed or async operations, answer:
- Who can cancel this operation?
- How do they receive the cancel signal?
- Therefore, who must own the timer/state?

This prevents the mistake of putting timers on the wrong side of a client-server boundary.
</if>
</if>
...
<if feature involves visual/game/scene changes:>
### E2E Test Sequence
Create a YAML test file at `docker/app/client/tests/e2e/<feature-name>.yaml` to verify visual behavior. Use the test-phaser schema (see `.claude/agents/test-phaser.md`) with steps for:
- Screenshots at key states
- Key presses or mouse interactions to trigger the feature
- `eval` checks for game state validation
</if>
</if>
