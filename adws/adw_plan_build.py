...
from utils import make_adw_id, setup_logger, get_worktree_path
...
AGENT_INIT_WORKTREE = "init_worktree"
AGENT_CLEAN_WORKTREE = "clean_worktree"
...
def build_plan(
    issue: GitHubIssue, command: str, adw_id: str, logger: logging.Logger, worktree_path: Optional[str] = None
) -> AgentPromptResponse:
    """Build implementation plan for the issue using the specified command."""
    logger.debug(f"[{adw_id[:6]}] Executing sdlc_planner in worktree")
...
def get_plan_file(
    plan_output: str, adw_id: str, logger: logging.Logger, worktree_path: Optional[str] = None
...
def implement_plan(
    plan_file: str, adw_id: str, logger: logging.Logger, worktree_path: Optional[str] = None
) -> AgentPromptResponse:
    """Implement the plan using the /implement command."""
    logger.debug(f"[{adw_id[:6]}] Executing sdlc_implementor in worktree")
...
def git_branch(
    ...
    """Generate and create a git branch for the issue with worktree.
    Returns (branch_name, worktree_path, error_message) tuple."""
    ...
    # Initialize worktree for the branch
    worktree_path, error = init_worktree(branch_name, adw_id, logger)

    if error:
        return branch_name, None, error

    return branch_name, worktree_path, None
...
def init_worktree(
    branch_name: str, adw_id: str, logger: logging.Logger
) -> Tuple[Optional[str], Optional[str]]:
    """Initialize a git worktree with sparse checkout for the branch.
    Returns (worktree_path, error_message) tuple."""
    logger.info(
        f"[{adw_id[:6]}] Creating worktree '{branch_name}' with sparse checkout of 'docker/app'"
    )

    request = AgentTemplateRequest(
        agent_name=AGENT_INIT_WORKTREE,
        slash_command="/init_worktree",
        args=[branch_name, "docker/app"],
        adw_id=adw_id,
        model="sonnet",
    )
    ...
    if not response.success:
        logger.error(
            f"[{adw_id[:6]}] Failed to create worktree '{branch_name}': {response.output}"
        )
        return None, response.output

    logger.info(f"[{adw_id[:6]}] Worktree ready at: {worktree_path}")
    logger.debug(f"Worktree path: {worktree_path}")
    return worktree_path, None


def clean_worktree(branch_name: str, adw_id: str, logger: logging.Logger) -> None:
    """Clean up a git worktree and its branch.
    This is best-effort cleanup - errors are logged but don't fail the workflow."""
    logger.info(f"[{adw_id[:6]}] Cleaning up worktree '{branch_name}'")

    request = AgentTemplateRequest(
        agent_name=AGENT_CLEAN_WORKTREE,
        slash_command="/clean_worktree",
        args=[branch_name],
        adw_id=adw_id,
        model="sonnet",
    )
    ...
    if not response.success:
        logger.warning(f"[{adw_id[:6]}] Cleanup warning: {response.output}")
    else:
        logger.info(f"[{adw_id[:6]}] Worktree '{branch_name}' cleaned up successfully")
...
def main():
    ...
    branch_name, worktree_path, error = git_branch(issue, issue_command, adw_id, logger)

    check_error(error, issue_number, adw_id, "ops", "Error creating branch/worktree", logger)
    ...
    # Wrap workflow in try/finally to ensure cleanup
    try:
        ...
        issue_plan_response: AgentPromptResponse = build_plan(
            issue, issue_command, adw_id, logger, worktree_path
        )
        ...
        plan_file_path, error = get_plan_file(issue_plan_response.output, adw_id, logger, worktree_path)
        ...
        commit_msg, error = git_commit(AGENT_PLANNER, issue, issue_command, adw_id, logger, worktree_path)
        ...
        implement_response: AgentPromptResponse = implement_plan(
            plan_file_path, adw_id, logger, worktree_path
        )
        ...
        commit_msg, error = git_commit(
            AGENT_IMPLEMENTOR, issue, issue_command, adw_id, logger, worktree_path
        )
        ...
        pr_url, error = pull_request(branch_name, issue, plan_file_path, adw_id, logger, worktree_path)
        ...
        # Clean up worktree regardless of success or failure
        if branch_name:
            clean_worktree(branch_name, adw_id, logger)
