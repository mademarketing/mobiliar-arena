...
def prompt_claude_code(request: AgentPromptRequest) -> AgentPromptResponse:
    ...
    # Determine working directory: use worktree_path if provided, otherwise current directory
    working_directory = request.worktree_path if request.worktree_path else None

    try:
        # Execute Claude Code and pipe output to file
        with open(request.output_file, "w") as f:
            result = subprocess.run(
                cmd, stdout=f, stderr=subprocess.PIPE, text=True, env=env, cwd=working_directory
            )
