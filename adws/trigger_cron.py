...
# Persistence file for tracking processed issues across restarts
PROCESSED_FILE = Path(__file__).parent / ".processed_issues.json"
...
def load_processed_issues() -> Set[int]:
    """Load previously processed issues from persistence file."""
    if PROCESSED_FILE.exists():
        try:
            data = json.loads(PROCESSED_FILE.read_text())
            loaded_issues = set(data)
            print(f"INFO: Loaded {len(loaded_issues)} previously processed issues from {PROCESSED_FILE}")
            return loaded_issues
        except (json.JSONDecodeError, ValueError) as e:
            print(f"WARNING: Failed to load processed issues file: {e}")
            return set()
    return set()


def save_processed_issues():
    """Save processed issues to persistence file."""
    try:
        PROCESSED_FILE.write_text(json.dumps(list(processed_issues), indent=2))
        print(f"INFO: Saved {len(processed_issues)} processed issues to {PROCESSED_FILE}")
    except Exception as e:
        print(f"ERROR: Failed to save processed issues: {e}")
