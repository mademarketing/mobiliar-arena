...
```
  ...
  - [More config parameters...](https://docs.phaser.io/phaser/concepts/tweens#creating-a-tween)
- Get value
...
- `tweens` : Array of [tween config](https://docs.phaser.io/phaser/concepts/tweens#creating-a-tween)
...
hold
  ↓
Is yoyo? → No → Repeat count > 0?
  ↓ Yes           ↓ Yes
Callback: onYoyo  Callback: onRepeat
  ↓               ↓
Tween backword   repeatDelay
Callback: onUpdate  ↓
(duration)        (back to delay)
  ↓
Repeat count > 0? → No → Loop count > 0?
  ↓ Yes                   ↓ Yes
Callback: onRepeat        Callback: onLoop
  ↓                       ↓
repeatDelay              loopDelay
  ↓                       ↓
(back to delay)          (back to delay)
                          ↓ No
                        completeDelay
                          ↓
                        Callback: onComplete
