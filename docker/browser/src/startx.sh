...
echo "--- StartX Session ---"
echo "Listing ALSA Playback Devices (after X server start):"
aplay -l || echo "Warning: 'aplay -l' command failed or no devices found."
echo "------------------------------------"
...
if [[ ! -z $FORCE_HD ]] && [[ "$FORCE_HD" -eq "1" ]]
then
  echo "Force HD Resolution"
