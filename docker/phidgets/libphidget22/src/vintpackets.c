...
VINTPacketStatusCodeDeviceCode_to_PhidgetReturnCode(VINTPacketStatusCode code, const PhidgetUniqueDeviceDef *udd) {
	...
	case PHIDUID_DCC1101:
		switch (code) {
		// XXX - this is just an example, replace with real errors
		case VINTPacketStatusCode_DCC1101_BADPOWER:
			return (EPHIDGET_BADPOWER);
		case VINTPacketStatusCode_DCC1101_POWERCYCLE:
			return (EPHIDGET_POWERCYCLE);
		case VINTPacketStatusCode_DCC1101_HALL_ERROR:
			return (EPHIDGET_HALLSENSOR);
		}
		break;
#endif
#if PHIDUID_DCC1004_SUPPORTED
	case PHIDUID_DCC1004:
		switch (code) {
			// XXX - this is just an example, replace with real errors
		case VINTPacketStatusCode_DCC1004_BADPOWER:
			return (EPHIDGET_BADPOWER);
		case VINTPacketStatusCode_DCC1004_POWERCYCLE:
			return (EPHIDGET_POWERCYCLE);
		}
		break;
#endif
...
sendDCC1004(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_DCC1004_DCMOTOR_100:
		...
		case BP_SETMOTORINDUCTANCE:
			packfloat(buf, (getBridgePacketDouble(bp, 0)));
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DCMOTOR_SETMOTORINDUCTANCE, buf, 4));
		case BP_MOTORDRIVETYPE:
			buf[0] = (uint8_t)getBridgePacketInt32(bp, 0);
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DCMOTOR_SETDRIVETYPE, buf, 1));
		default:
			MOS_PANIC("Unexpected packet type");
		}
	...
	case PHIDCHUID_DCC1101_BLDCMOTOR_100:
		...
		case VINT_PACKET_TYPE_MOTORCONTROLLER_FAILSAFE:
			...
			case 3:
				...
				case 7:
					return (PhidgetChannel_sendErrorEvent(ch, EEPHIDGET_BADCONNECTION, "Hall sensor error. The Hall sensor is likely damaged or unplugged."));
				}
			case 4:
